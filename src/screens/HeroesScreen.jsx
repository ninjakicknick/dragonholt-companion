...
export default function HeroesScreen({ heroes, editingHero, setEditingHero, createHero, updateHero, deleteHero, toggleHeroSkill, toggleDisabledSkill }) {
        ...
        <div className="bg-[#f8f4e6] p-4 sm:p-6 rounded-lg shadow-md border border-[#d2c2a5] space-y-6">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end justify-between border-b border-[#d2c2a5] pb-4">
            <div className="w-full sm:w-auto flex-1">
              ...
              <div className="flex gap-2 mt-2">
                <input type="text" value={hero.race} onChange={event => updateHero(hero.id, { race: event.target.value })} className="text-[#8c2a2a] bg-transparent border-b border-[#d2c2a5] focus:outline-none" placeholder="Race" />
                <span className="text-[#8c2a2a] font-bold">|</span>
                <input type="text" value={hero.class} onChange={event => updateHero(hero.id, { class: event.target.value })} className="text-[#8c2a2a] bg-transparent border-b border-[#d2c2a5] focus:outline-none" placeholder="Class" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-3 rounded border border-[#d2c2a5] text-center">
              ...
              <div className="mt-2 text-xs flex justify-center items-center gap-1"><span>Max:</span><input type="number" value={hero.maxStamina} onChange={event => updateHero(hero.id, { maxStamina: Number.parseInt(event.target.value, 10) || 0 })} className="w-10 text-center border-b" /></div>
            </div>
            <div className="bg-white p-3 rounded border border-[#d2c2a5] text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><h4 className="font-bold text-[#4a3b32] mb-2 border-b border-[#d2c2a5]">Background / Notes</h4><textarea value={hero.notes} onChange={event => updateHero(hero.id, { notes: event.target.value })} className="w-full h-24 p-2 bg-white border border-[#d2c2a5] rounded text-sm focus:outline-none" placeholder="Personality, ideals, appearance..." /></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      <button onClick={createHero} className="w-full py-3 bg-[#8c2a2a] hover:bg-[#6b1e1e] text-white rounded-lg font-bold flex items-center justify-center gap-2 transition-colors shadow"><Plus size={20} /> Create New Hero</button>
