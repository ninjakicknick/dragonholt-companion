    ...
    <div className="overflow-x-auto bg-[#f8f4e6] p-4 rounded-lg shadow-inner border border-[#d2c2a5]">
      <div className="min-w-max">
        <div className="grid grid-cols-9 gap-1 mb-1">
          ...
          {NUMBERS.map(number => (
            <div key={number} className="w-8 h-8 flex items-center justify-center font-bold text-[#5c4a3d]">
              {number}
            </div>
          ))}
        </div>
            ...
            {NUMBERS.map(number => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  aria-label={`${active ? 'Remove' : 'Add'} story point ${point}`}
                  className={`w-8 h-8 text-xs rounded border transition-colors ${active
